...
**GitOps Promoter metrics:** Upstream **`gitops-promoter`** Helm **0.5.1** ships a **`ServiceMonitor`** whose selector expects **`app.kubernetes.io/name: gitops-promoter`** on the metrics **Service**, but that **Service** is labeled **`app.kubernetes.io/name: service`**, so Prometheus **discovers endpoints then drops every target** at relabel time—**`git_operations_*`** and related series never appear and the **GitOps Promoter** Grafana dashboard looks empty. This repo sets **`gitops-promoter.prometheus.enable: false`** and installs a corrected **`ServiceMonitor`** from **`charts/gitops-promoter/templates/controller-metrics-servicemonitor.yaml`**. A bundled Grafana dashboard is in **`charts/monitoring/`** (**`gitops-promoter.json`**), with panels for **`git_operations_*`**, **`scm_calls_*`**, webhook histograms, **`application_watch_events_handled_total`**, and **`controller_runtime_reconcile_total`**. If panels stay empty after a config change, use **Explore → Prometheus** with **`up{namespace=\"gitops-promoter\"}`** or **`git_operations_total`** and widen the time range; you can **`kubectl -n monitoring rollout restart deploy/monitoring-kube-prometheus-operator`** if new **`ServiceMonitor`** objects are slow to appear in Prometheus.
...
Prometheus is configured with **`serviceMonitorSelectorNilUsesHelmValues: false`** and an empty **`serviceMonitorSelector`**, so it discovers **`ServiceMonitor`** resources from other Helm releases (for example **`gitops-promoter`** in namespace **`gitops-promoter`**), not only monitors labeled **`release: monitoring`**. Members of GitHub team **`@argoproj-labs/gitops-promoter-approvers`** get Grafana org role **Editor** via **`auth.github.role_attribute_path`**; everyone else stays **Viewer**. After changing role mapping, users should **sign out and sign in again** so Grafana refreshes roles from GitHub.
